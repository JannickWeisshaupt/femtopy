...
import numpy as np
from pyterminal.terminal_frame import TerminalFrame
...
db = DB.Database(r"RefractiveIndex.db")
...
class EmbeddedFigure(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.options_dict = FixedDict(
            {'logarithmic': [False, False], 'xlim': None, 'ylim': None, 'eV': False, 'k': True})
...
class RefrectiveIndexHandler(object):
    ...
    def get_data_from_id(self, id):
        ...
        self.wavelength = var1[:, 0].astype(np.float)
        self.n = var1[:, 1].astype(np.float)
    ...
    def __repr__(self):
        return 'RefractiveIndexHandler Object at '+hex(id(self))+ \
               ' of material:'+self.name+'\nContains variables:\nn: refractive index\nwavelength: wavelength\nk: absorption coefficient\ngvd: Group velocity dispersion'
...
class WholeDatabaseFrame(tk.Toplevel):
    def __init__(self, master, *args, **kwargs):
        ...
        for shelf in shelf_list:
            ...
            for book in set_of_books:
                s_res = db.search_custom(
                    "SELECT * FROM pages WHERE shelf LIKE '" + shelf + "' and book like '" + book + "'")
                if len(s_res) > 1:
                    y1 = self.result_tree.insert(x1, 'end', text=book, values=())
                    for i, res in enumerate(s_res):
                        self.result_tree.insert(y1, 'end', text='',
                                                values=(
                                                res[2], res[3], res[7], res[8], ('No', 'Yes')[res[6]], res[9], res[0]))
                else:
                    res = s_res[0]
                    self.result_tree.insert(x1, 'end', text='', values=(
                    res[2], res[3], res[7], res[8], ('No', 'Yes')[res[6]], res[8], res[0]))
    ...
    def click_event(self, event):
        ...
        for i, item_1 in enumerate(selection_list):
            ...
            if i > 20:
                tk.messagebox.showerror('Too many selected',
                                        'You have more than twenty selected which is imho too much')
                break
...
class SearchFrame(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.result_tree.heading('#7', text='Db number', anchor=tk.CENTER,
                                 command=lambda: treeview_sort_column(self.result_tree, '#7', False))
        self.result_tree.column("#7", minwidth=50, width=130, stretch=tk.NO)
    ...
    def search(self):
        ...
        if exact_bool:
            ...
            for sterm in stlist:
                if sterm == '':
                    continue
                sql_query = "SELECT * FROM pages WHERE " + search_area + " like '%" + sterm + "%'"
                search_res_small = db.search_custom(sql_query)
                self.search_res.extend(search_res_small)
...
class ShortPulseWindow(tk.Toplevel):
    def __init__(self, master, *args, **kwargs):
        super().__init__(master, *args, **kwargs)
        self.master = master

        self.ref_handler = master.list_of_refractive_index[0]
        ...
        self.lamda0_entry = LabelWithEntry(self.frame0,'Lambda [mum]:',width_label=width_label1, width_entry=width_entry)
        self.lamda0_entry.set('0.8')
        self.lamda0_entry.grid(row=0,column=0,padx=5)
        self.lamda0_entry.bind('<Return>',lambda event: self.update_figure())

        self.dT_entry = LabelWithEntry(self.frame0,'Time duration [fs]:',width_label=width_label1, width_entry=width_entry)
        self.dT_entry.set('50')
        self.dT_entry.grid(row=1,column=0,padx=5)
        self.dT_entry.bind('<Return>',lambda event: self.update_figure())

        self.thickness_entry = LabelWithEntry(self.frame0,'Material thickness [mm]:',width_label=width_label2, width_entry=width_entry)
        self.thickness_entry.set('1')
        self.thickness_entry.grid(row=0,column=1)
        self.thickness_entry.bind('<Return>',lambda event: self.update_figure())

        self.f = plt.Figure(figsize=(10, 6))
        gs = gridspec.GridSpec(2, 1, height_ratios=[2, 1])
        self.subplot1 = self.f.add_subplot(gs[0])
        self.subplot2 = self.f.add_subplot(gs[1])
        self.subplot1.format_coord = lambda x, y: "lambda={0:1.2f}, GVD={1:1.0f}".format(x, y)
        self.subplot2.format_coord = lambda x, y: "lambda={0:1.2f}, GVD={1:1.0f}".format(x, y)
    ...
    def update_figure(self):
        ...
        c0 = 3e8
        lambda0 = float(self.lamda0_entry.get())
        omega0 = np.pi*2*c0/(lambda0*1e-6)
        d_mat = float(self.thickness_entry.get())*1e-3
        dT = float(self.dT_entry.get())*1e-15
        n = self.ref_handler.n
        k = self.ref_handler.k
        if k is None:
            k = 0*n
        wavelength_n = self.ref_handler.wavelength

        omega_max = 2 * np.pi * c0 / (wavelength_n.min()*1e-6)
        omega_min = 2 * np.pi * c0 / (wavelength_n.max()*1e-6)
        # spec_width = 0.44*(lambda0*1e-6)**2/(c0*dT)*1e6
        spec_width = 2*np.pi*0.441/dT

        omega = np.linspace(omega_min, omega_max, 10000)
        wav = 2 * np.pi * c0 / omega * 1e6

        n_int = np.interp(wav,wavelength_n, n)
        k_int = np.interp(wav, wavelength_n, k);

        index0 = np.argmin(np.abs(wav-lambda0))
        n0 = n_int[index0]

        spectrum = np.exp(-np.log(2)*(omega - omega0) ** 2 / spec_width ** 2)
        time_domain = np.fft.fftshift(np.fft.ifft(spectrum))

        spectrum_mod = spectrum * np.exp(-1j * (n_int-n0-1j*k_int) * omega * d_mat / c0)
        time_domain_mod = np.fft.fftshift(np.fft.ifft(spectrum_mod))

        time = np.fft.fftshift(np.fft.fftfreq(len(spectrum), omega[1] - omega[0]))


        self.subplot1.plot(wav, np.abs(spectrum) ** 2)

        self.subplot2.plot(time, np.abs(time_domain) ** 2)
        # self.subplot2.plot(time, np.abs(time_domain_mod) ** 2)

        self.subplot2.plot(time, np.roll(np.abs(time_domain_mod) ** 2, len(time) // 2 - np.argmax(np.abs(time_domain_mod) ** 2)))
...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        master.geometry('{}x{}'.format(1100, 920))
        ...
        self.short_pulse_window = None

        self.create_menubar()
        self.bind_global_commands()

        master.title('Refractive Index Database Viewer')
    ...
    def create_menubar(self):
        ...
        self.menubar.add_cascade(label="Calculations", menu=self.calcmenu)

        self.calcmenu.add_command(label='Python terminal', command=self.open_terminal_frame)
        self.calcmenu.add_command(label='Short Pulse', command=self.open_short_pulse_window)


    def open_terminal_frame(self):
        if self.terminal_frame is None or not self.terminal_frame.winfo_exists():
            shared_vars = {'refractive_indexes': self.list_of_refractive_index}
            welcome_message = """

Example:

plt.figure(1)
for ref in refractive_indexes:
    plt.plot(ref.wavelength,ref.n) 
"""
            self.terminal_frame = TerminalFrame(self, shared_vars,deepcopy=False,welcome_message=welcome_message)
    ...
    def open_short_pulse_window(self):
        if self.short_pulse_window is None or not self.short_pulse_window.winfo_exists():
            if self.list_of_refractive_index is not None and len(self.list_of_refractive_index) == 1:
                self.short_pulse_window = ShortPulseWindow(self)
            else:
                tk.messagebox.showerror("Error",'Please choose exactly one refractive index')
